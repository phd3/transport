...
public class FunctionDependencies
{
    ...
    // Transport library requires the global metadata to convert generic types to trino types and vice-versa.
    // Through this hack, we are allowing transport library to access the metadata via the SqlScalarFunction interface
    ...
    public FunctionInvoker getFunctionInvoker(
        QualifiedName name, List<Type> parameterTypes, InvocationConvention invocationConvention)
    {
        FunctionKey functionKey = new FunctionKey(name, toTypeSignatures(parameterTypes));
        ResolvedFunction resolvedFunction = functions.get(functionKey);
        if (resolvedFunction == null) {
    ...
    public FunctionInvoker getOperatorInvoker(
        OperatorType operatorType, List<Type> parameterTypes, InvocationConvention invocationConvention)
    {
        OperatorKey operatorKey = new OperatorKey(operatorType, toTypeSignatures(parameterTypes));
        ResolvedFunction resolvedFunction = operators.get(operatorKey);
        if (resolvedFunction == null) {
    ...
    public FunctionInvoker getCastSignatureInvoker(
        TypeSignature fromType, TypeSignature toType, InvocationConvention invocationConvention)
    {
